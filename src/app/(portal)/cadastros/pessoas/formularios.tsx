...
import type { MembershipRole, Unit } from '@/lib/types';
...
export function FormularioDePessoa({ unidades }: { unidades: Unit[] }) {
  ...
  return (
    <form action={acao} className="grid gap-4 sm:grid-cols-6">
      <div className="sm:col-span-3">
        <Field label="Nome">
          <Input name="nome" defaultValue={digitado.nome} required />
        </Field>
      </div>

      <div className="sm:col-span-3">
        <Field label="E-mail" hint="Necessário para enviar boleto e convite de acesso">
      ...
      <div className="sm:col-span-2">
        <Field label="CPF" hint="Conferido pelo dígito verificador">
          <Input name="cpf" defaultValue={digitado.cpf} placeholder="000.000.000-00" />
        </Field>
      </div>

      <div className="sm:col-span-2">
        <Field label="Telefone">
          <Input name="telefone" defaultValue={digitado.telefone} placeholder="(31) 99999-8888" />
        </Field>
      </div>

      <div className="sm:col-span-2">
        <Field label="Papel">
          <Select key={digitado.papel ?? 'Resident'} name="papel" defaultValue={digitado.papel ?? 'Resident'}>
            {PAPEIS.map((papel) => (
      ...
      <div className="sm:col-span-2">
        <Field label="Unidade" hint="Opcional; pode vincular depois">
          <Select key={digitado.unidadeId ?? ''} name="unidadeId" defaultValue={digitado.unidadeId ?? ''}>
      ...
      <div className="sm:col-span-2">
        <Field label="Relação com a unidade">
          <Select key={digitado.relacao ?? 'Owner'} name="relacao" defaultValue={digitado.relacao ?? 'Owner'}>
      ...
      <div className="flex items-end sm:col-span-2">
        <label className="flex items-center gap-2 pb-2 text-sm text-ink">
          <input
            type="checkbox"
            name="responsavel"
            defaultChecked={estado.valores ? digitado.responsavel !== undefined : true}
...
export function SeletorDePapel({
  ...
  return (
    <form action={acao}>
      ...
      {estado.erro ? (
        <span className="mt-1 block max-w-xs text-xs text-negative" role="alert">
          {estado.erro}
        </span>
      ) : null}
    </form>
...
export function BotaoDeConvite({
  ...
  return (
    <div>
      <form action={acao}>
        <input type="hidden" name="pessoaId" value={pessoaId} />
        <Button type="submit" variant="secondary" disabled={enviando} className="py-1 text-xs">
          {rotulo}
        </Button>
      </form>
  ...
  return (
    <div>
      <form action={acao} className="flex flex-wrap items-end gap-2">
        ...
        <Select name="unidadeId" aria-label="Unidade" className="w-auto py-1.5 text-xs">
          {unidades.map((unidade) => (
            <option key={unidade.id} value={unidade.id}>
              {unidade.fullIdentifier}
            </option>
        ...
        <Select name="relacao" aria-label="Relação" className="w-auto py-1.5 text-xs">
          <option value="Owner">Proprietário</option>
          <option value="Tenant">Inquilino</option>
          <option value="Occupant">Morador</option>
        </Select>
...
export function BotaoDesvincular({ vinculoId }: { vinculoId: string }) {
  ...
  return (
    <form action={acao} className="inline">
      ...
      {estado.erro ? <span className="ml-1 text-xs text-negative">{estado.erro}</span> : null}
    </form>
  );
}
